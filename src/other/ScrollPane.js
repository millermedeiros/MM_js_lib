define(['./TouchController', 'lib/js-signals'], function(TouchController, signals){	
	...
	function ScrollPane(frameElm, contentHolder, numPanes){
		...
		this._touchController = new TouchController(this._frame, this);
		this._setup();
	...
	ScrollPane.prototype = {
		...
		_setup : function(){
			...
			tc.onTouchStart = this._touchStartHandler;
			tc.onTouchMove = this._touchMoveHandler;
			tc.onTouchEnd = this._touchEndHandler;
		},
		...
		_touchEndHandler : function(evt, changePos, momentum, duration){
			...
			this.goTo(this.currentPane + inc, true);
		},
